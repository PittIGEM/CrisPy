...
class Sequalizer(object):
	...
	def get_mutation_freq(self, match_override=None):
		...
		for base_index in self.target_range:
			...
			if match_override is not None:
				target_base = self.ref_data['sequence'][self.align_start+base_index]
			else:
				target_base = self.target_sequence[base_index]
			# calculates the muation frequency for each target base
			if target_base == 'G':
				trace_range = [self.ref_data['base_pos'][self.align_start+base_index]-2, self.ref_data['base_pos'][self.align_start+base_index]+2]
...
if __name__ == "__main__":
	...
	root = tk.Tk()
	# root.withdraw()
	ref_path = filedialog.askopenfilename(title = "Select reference file", filetypes = [("ab1 files","*.ab1")])
	test_path = filedialog.askopenfilename(title = "Select experiment file", filetypes = [("ab1 files","*.ab1")])
	target_sequence = simpledialog.askstring("Target Sequence","Input your nucleotide sequence:")
	...
	# Input Data
	target_sequence = 'CCGGCAAGCTGCCCGTGCCC'
	target_range = [13,14,15,16]

	# Initializes SeqDoc object with reference and test file
	a = SeqDoc(ref_path, test_path)
	align_length, diffs = a.get_all_data()

	# Initializes OffTargetFinder
	c = OffTargetFinder(a.ref_trace,target_sequence)
	match_dict = c.get_targets()

	# Initializes Sequalizer object
	b = Sequalizer(a.ref_trace, a.test_trace, diffs, target_sequence, target_range)
	print(b.get_mutation_freq())
	for k in sorted(match_dict):
		print(k)
		print(b.get_mutation_freq(match_override=match_dict[k]))

	# # dispplay difference data on a plot
	# plt.xticks(a.ref_trace['base_pos'])
	# plt.plot(diffs['A'])
	# plt.plot(diffs['G'])
	# plt.plot(diffs['C'])
	# plt.plot(diffs['T'])
